...
class HttpProxyMiddleware(object):
    ...
    def fetch_new_proxyes(self):
        ...
        new_proxyes = fetch_free_proxyes.fetch_all(https=True)
    ...
    def process_response(self, request, response, spider):
        ...
        if "proxy" in request.meta.keys():
            logger.info("%s %s %s" % (request.meta["proxy"], response.status, request.url))
        else:
            logger.info("None %s %s" % (response.status, request.url))

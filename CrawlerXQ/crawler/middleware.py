...
class HttpProxyMiddleware(object):
    ...
    def __init__(self):
        self.proxyes = ["", "http://10.180.85.116:8123"]
        self.index = 0  #0表示不用代理， 直接连接

    def using_proxy(self, request):
        if self.index != 0:
            proxy = self.proxyes[self.index]
            logger.info("using HTTP_PROXY: %s" % proxy)
            request.meta["proxy"] = proxy
        

    def process_request(self, request, spider):
        self.using_proxy(request)
                
    def process_response(self, request, response, spider):
         logger.debug("process response: HttpProxy")
         if (response.body).find('请输入验证码 - 雪球')>=1:
            reason = 'BAN'  
            logger.info("HttpProxy: Captch Error")
            if spider.name != 'TestSpider':
                new_request = request.copy()
                self.index = (self.index+1)%len(self.proxyes)
                logger.debug("index: %d" % self.index)
                self.using_proxy(new_request)
                return new_request
...
#         self.priority_adjust = settings.getint('RETRY_PRIORITY_ADJUST')
    ...
    def process_response(self, request, response, spider):
        logger.debug("RetryMiddleware process_reponse")
        ...
        if (response.body).find('error_description') >= 1:
            ...
            if d_json['error_description']:
                ...
                logger.info(reason)
                if reason=='用户不存在':
                    log.msg('User Not Exists: '+str(request))
                    raise IgnoreRequest
                elif reason=='该组合不存在':
                    # so many cubes not exists anymore, avoid output
                    # log.msg('Cube Not Exists: '+str(request))
                    raise IgnoreRequest
                elif reason=='用户未登录':
                    log.msg('User Not Login: '+str(request))
                    raise IgnoreRequest
                elif reason=='用户id无效':
                    log.msg('Invalid User ID: '+str(request))
                    raise IgnoreRequest
                elif reason=="无效的组合id":  # 原来参加实盘的人不参加了
                    log.msg('Invalid GWS/CUBE ID: '+str(request))
                elif reason=="请求分页超过了最大限制":  # cube_rb的实际页数小于json中的maxPage
                    log.msg('Out of max page: '+str(request))
                    raise IgnoreRequest
                elif reason=="Runtime unknown internal error":
                    log.msg("Runtime unknown internal error"+str(request))
                    raise IgnoreRequest
                else:
    ...
    def _retry(self, request, reason, spider, custom_retry_times, retry_interval):
        ...
        if retries <= custom_retry_times:
            ...
            log.msg("Gave up retrying %(request)s \n(failed %(retries)d / %(max_retry)s times): %(reason)s" 

...
class XQUserCmtSpider(BaseSpider):
    ...
    def start_requests(self):
        ...
        self.cursor.execute('''select distinct fl.uid from xq_user_followers as fl
                                        where fl.fl_uid in (select distinct ginfo.uid from xq_gws_info as ginfo where valid=1)
                                        and  fl.uid not in (select distinct ginfo.uid from xq_gws_info as ginfo where valid=1);''')
    ...
    def parse_cmt(self, response):
        try:
            ...
            if list:
                item=XQCmtItem()
                for s in list:
                    item['item_name']='xq_user_statuses'
                    item['cmt_id']=s['id']
                    item['uid']=s['user_id']
